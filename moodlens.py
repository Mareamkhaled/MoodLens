...
#         "upset": ["sad", "hurt", "disappointed", "زعلان", "تعبان", "tired", "heartbroken", "exhausted", "drained", "lonely", "down", "بالأسى", "حزين", "مريض", "قلق","مهموم"]
#     }
...
#     # Arabic fallback
#     if lang == "ar":
#         if any(word in lower_text for word in ["زعلان", "تعبان", "بالأسى", "حزين", "مريض", "قلق", "مهموم"]):
#            return "upset"
#         elif any(word in lower_text for word in ["فرحان", "مبسوط", "سعيد"]):
#            return "joyful"
#         else:
#            return "neutral"
...
import re
from emotion_model import detect_model_emotion
from entity_detection import extract_entities
from suggestions import get_activity_suggestion
from colors import emotion_colors
from empathy import get_empathetic_message, get_follow_up_question
...
def analyze_emotion(text):
    if is_arabic(text):
        return {
            "error": "Please enter your message in English only."
        }

    emotion = detect_model_emotion(text)
    entities = extract_entities(text)
    activity = get_activity_suggestion(emotion, entities)
    color = emotion_colors.get(emotion, "#D3D3D3")
    message = get_empathetic_message(emotion)
    follow_up = get_follow_up_question(emotion)
